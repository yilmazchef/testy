...
public class ExamRepository implements IExamRepository {
    ...
    public Boolean exists(ExamEntity exam) {
        ...
        String sql = "SELECT COUNT(*) FROM testy_exam WHERE (session = ?) AND (studentId = ?) AND (taskId = ?) AND (active = TRUE) ";
        EXAM_LOGGER.info(sql);
        Long count = jdbcTemplate.queryForObject(sql, Long.class, exam.getSession(), exam.getStudentId(), exam.getTaskId());
        return count > 0;
    }
    ...
    public Boolean exists(String session, String studentId, String questionId) {
        ...
        String sql = "SELECT COUNT(*) FROM testy_exam WHERE active = TRUE AND (testy_exam.session = ?) AND (testy_exam.studentId = ?) AND (testy_exam.taskId IN (SELECT testy_task.id FROM testy_task WHERE testy_task.questionId = ?) ) ";
        EXAM_LOGGER.info(sql);
    ...
    public String save(ExamEntity exam) throws SQLException {
        ...
        String sql = "INSERT INTO testy_exam " +
                "(id, code, session, studentId, taskId, startTime, endTime, score, selected)" +
                " " + "VALUES" + " " +
                "(?, ?, ?, ?, ?, ?, ?, ?, ?)";
        EXAM_LOGGER.info(sql);

        if (exists(exam).equals(Boolean.TRUE)) throw new SQLException("Exam already exists.");

        if (exam.getId() == null) exam.setId(UUID.randomUUID().toString());
        int insertCount = jdbcTemplate.update(sql,
                exam.getId(), exam.getCode(), exam.getSession(), exam.getStudentId(), exam.getTaskId(), exam.getStartTime(), exam.getEndTime(), exam.getScore(), exam.getSelected()
        );

        if (insertCount <= 0) throw new SQLException("Exam could not be saved.");
    ...
    public String update(ExamEntity exam) throws SQLException {
        ...
        String sql = "UPDATE testy_exam" +
                " SET " +
                "(session = ?), (code = ?), (studentId = ?), (taskId = ?), (startTime = ?), (endTime = ?), (score = ?), (active = ?), (selected = ?)" +
                " WHERE " +
                "(id = ?)";
        EXAM_LOGGER.info(sql);
        int updateCount = jdbcTemplate.update(sql,
                exam.getSession(), exam.getCode(), exam.getStudentId(), exam.getTaskId(), exam.getStartTime(), exam.getEndTime(), exam.getScore(), exam.getActive(), exam.getSelected(),
                exam.getId()
        );
    ...
    public String patch(String examId, Boolean isSelected) throws SQLException {
        ...
        if (exists(examId).equals(Boolean.FALSE)) throw new SQLException("Exam does NOT exists.");
    ...
    public String patchTask(String taskId, String session, Boolean isSelected) throws SQLException {
        ...
        if (existsByTask(taskId, session).equals(Boolean.FALSE)) throw new SQLException("Task does NOT exists.");
    ...
    public List<String> patchSession(String examCode, String session) throws SQLException {
        if (existsByCode(examCode).equals(Boolean.FALSE)) throw new SQLException("Exams NOT found.");
